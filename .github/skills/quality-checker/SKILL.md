...
docker exec -i native-platform /bin/bash -c "
  ...
  make -j\$(nproc) && \
  find unittest uploadstblogs/unittest -type f -executable -name '*gtest*' 2>/dev/null | while read test_bin; do
...
docker exec -i native-platform /bin/bash -c "
  ...
  if [ -f 'uploadstblogs/src/uploadstblogs' ]; then
    ls -lh uploadstblogs/src/uploadstblogs
    file uploadstblogs/src/uploadstblogs
  fi
"
```
...
**User**: "Full analysis on uploadstblogs/src"
- Run all checks scoped to uploadstblogs directory
