...
|------|-----------|-------|----------|--------------|----------|----------|
| 1 | High-001 (DCM-001) | Unauthenticated, unvalidated config-file path accepted from RBUS Setconfig event — arbitrary file parse as root → heap overflow / RCE chain | High | Exploitable | CWE-73: External Control of File Name or Path | confPath from RBUS passed verbatim to fopen()/strcpy() with zero validation; chain to root RCE via cTimeZone→cRdkPath→popen() confirmed in source. Local-only vector, config-gated popen() branch. |
| 2 | High-002 (DCM-002) | Shell command injection via unescaped cRdkPath in popen() — reachable from XCONF/RBUS via adjacent-field heap overflow | High | Exploitable | CWE-78: OS Command Injection | cRdkPath[80] adjacent to cTimeZone[16]; ≥16-byte TimeZoneMode overwrites cRdkPath with shell metacharacters into popen() as root. Blocked only when ENABLE_MAINTENANCE is set. |
| 3 | High-003 (DCM-003) | Unbounded strcpy() of cloud-supplied JSON string values into tiny fixed heap buffers (8/16/128 bytes) | High | Exploitable | CWE-122: Heap-based Buffer Overflow | strcpy(sval, pJsonItem->valuestring) with no length check into 5 tiny buffers. Even legitimate cron strings (17 bytes) overflow logCron[16]. Network vector requires operator-infra compromise. |
| 4 | High-004 (DCM-004) | Heap overflow of cTimeZone[16] corrupts adjacent cRdkPath[80] → root command injection via popen() | High | Exploitable | CWE-78: OS Command Injection | End-to-end chain confirmed: JSON → strcpy overflow → cRdkPath corruption → popen() as root. ~100-byte payload suffices. Blocked when ENABLE_MAINTENANCE is set. |
| 5 | High-005 (DCM-005) | 1-byte destination stack buffer passed to strcpy() when UploadOnReboot is sent as a JSON string instead of expected boolean | High | Exploitable | CWE-121: Stack-based Buffer Overflow | INT8 temp=0 (1 byte) passed to strcpy(); no type enforcement. Adjacent-local corruption occurs before canary check. Stack-protector converts RCE to DoS on hardened builds. |
| 6 | High-006 (DCM-006) | popen() wrapper executes caller-supplied string via /bin/sh with no validation, escaping, or whitelisting | High | Exploitable | CWE-78: OS Command Injection | Terminal sink for the highest-impact chain. dcmUtilsCopyCommandOutput() passes cmd verbatim to popen(cmd,"r") with no sanitization. |
| 7 | High-007 (DCM-007) | Static-buffer overflow in attempt_proxy_fallback() while parsing server-supplied S3 URL — .bss overflow in root process | High | Exploitable | CWE-787: Out-of-bounds Write | path_len reaches ~1013 bytes, overflowing clean_path[512] by ~500 bytes. Local /tmp/httpresults.txt tamper vector is reliable due to umask(0). |
| 8 | High-008 (DCM-008) | Unbounded strcpy of network-sourced JSON values into 8/16/128-byte struct fields — heap metadata corruption | High | Exploitable | CWE-120: Classic Buffer Overflow | Duplicate root cause of DCM-003; difdCron[16] is last field of malloc'd DCMDHandle — overflow runs off heap chunk into allocator metadata. |
| 9 | High-009 (DCM-009) | Stack overflow of t2_ver[32] from RBUS-supplied string via unbounded strcpy in dcmRbusGetT2Version() | High | Exploitable | CWE-120: Classic Buffer Overflow | strcpy(t2_ver, rbus_string) into 32-byte stack buffer. RBUS parameter remotely writable via WebPA/RFC and persists across reboot. Stack-protector converts RCE to DoS. |
| 10 | High-010 (DCM-010) | Directory symlink following in add_directory_to_tar() allows arbitrary root-readable file exfiltration via uploaded archive | High | Exploitable | CWE-59: Improper Link Resolution Before File Access | stat() follows symlinks on directory entries under world-writable /tmp/DCM/; root-readable files archived and uploaded. Non-racy window after each upload cleanup. |
| 11 | High-011 (DCM-011) | gzopen() on predictable path under /tmp without O_NOFOLLOW/O_EXCL enables arbitrary file overwrite as root | High | Exploitable | CWE-377: Insecure Temporary File | Predictable filename (MAC + 1-min timestamp) in /tmp subdir; symlink causes root to truncate/overwrite arbitrary file. fs.protected_symlinks bypassed via attacker-owned subdir. |
| 12 | Medium-001 (DCM-012) | Stack buffer overflow in dcmRbusGetT2Version() via unbounded strcpy() of RBUS-supplied string | Medium | Exploitable | CWE-121: Stack-based Buffer Overflow | Same root cause as DCM-009; fires only at daemon init. Requires winning RBUS provider registration race. Stack canary converts RCE to DoS on hardened builds. |
| 13 | Medium-002 (DCM-013) | popen() executed on uninitialised heap buffer when DCM_LOG_UPLOAD scheduler fires before DCM_FW_UPDATE | Medium | Non-Exploitable | CWE-457: Use of Uninitialized Variable | malloc(1024) not zeroed; LOG_UPLOAD branch never writes pExecBuff but popen() is unconditional. Fresh sbrk pages are zero-filled. Realistic impact is DoS, not RCE. |
| 14 | Medium-003 (DCM-014) | umask(0) makes every file created by root daemon world-writable (mode 0666/0777) | Medium | Exploitable | CWE-732: Incorrect Permission Assignment for Critical Resource | Any local user can tamper /tmp/DCMSettings.conf to suppress uploads or rewrite PID file for DoS. Affects ~12 files. Impact is integrity tampering / DoS, not code execution. |
| 15 | Medium-004 (DCM-015) | Stack buffer overflow in dcmSettingSaveMaintenance() when cron minute/hour token ≥ 4 chars — even legal cron fields overflow | Medium | Exploitable | CWE-121: Stack-based Buffer Overflow | buffhr[4]/buffmin[4] written via *ptr++ with no bounds check. Even '*/15' (4 chars) overflows. Requires HAS_MAINTENANCE_MANAGER build + ENABLE_MAINTENANCE runtime. |
| 16 | Medium-005 (DCM-016) | Negative-index heap write before cJsonStr when 'urn:settings:TelemetryProfile' appears at offset 0/1 — heap metadata corruption | Medium | Non-Exploitable | CWE-787: Out-of-bounds Write | Writes two FIXED bytes (0x7D, 0x00) at FIXED offsets into glibc chunk header. Modern glibc aborts on size-mismatch. DoS only, no path to code execution. |
| 17 | Medium-006 (DCM-017) | Arbitrary file read as root via attacker-controlled confPath — no path validation | Medium | Exploitable | CWE-73: External Control of File Name or Path | fopen(confPath,"r") with zero path sanitization. Enables DoS via FIFO/huge file and pivots to all heap/stack overflows. Info leak constrained to JSON-parseable files. |
| 18 | Medium-007 (DCM-018) | Root writes /tmp/DCMSettings.conf via fopen("w") without O_NOFOLLOW/O_EXCL — symlink clobber + world-writable 0666 file | Medium | Non-Exploitable | CWE-59: Improper Link Resolution Before File Access | Symlink-clobber blocked by fs.protected_symlinks=1 (default on RDK/Yocto). 0666-tamper allows upload suppression only — integrity tampering, not code execution. |
| 19 | Medium-008 (DCM-019) | Unchecked CRON_INVALID_INSTANT (-1) return causes infinite tight loop firing root popen() — CPU exhaustion DoS | Medium | Exploitable | CWE-835: Infinite Loop | Impossible-date cron like "0 0 31 2 *" causes scheduler to fire dcmRunJobs() in tight loop. Impact limited to availability (CPU exhaustion, log growth, flash wear). |
| 20 | Medium-009 (DCM-020) | Symlink-follow on /tmp/.dcm-daemon.pid fopen("w") — arbitrary file truncate/create as root with mode 0666 | Medium | Non-Exploitable | CWE-59: Improper Link Resolution Before File Access | Blocked by fs.protected_symlinks=1 (default on modern kernels). Requires boot-time race. Kernel symlink protection neutralizes the attack on typical RDK builds. |
