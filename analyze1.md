...
|------|-----------|-------|----------|--------------|----------|----------|
| 1 | High-001 (DCM-001) | Unauthenticated, unvalidated config-file path accepted from RBUS Setconfig event — arbitrary file parse as root → heap overflow / RCE chain | High | Exploitable — local RBUS publisher can trigger heap overflow and RCE as root; requires rtrouted socket access and provider-registration race or raw rtMessage injection | CWE-73: External Control of File Name or Path | confPath from RBUS event is passed verbatim to fopen()/strcpy() with zero validation. Exploitability is local-only and requires bus-level trickery, but the chain to root RCE via cTimeZone→cRdkPath→popen() is fully confirmed in source. Downgraded from Critical to High due to local-only vector and config-gated popen() branch. |
| 2 | High-002 (DCM-002) | Shell command injection via unescaped cRdkPath in popen() — reachable from XCONF/RBUS via adjacent-field heap overflow | High | Exploitable — attacker-controlled cRdkPath is interpolated unescaped into popen("/bin/sh %s/swupdate_utility.sh ...") as root; blocked only when ENABLE_MAINTENANCE is set | CWE-78: OS Command Injection | cRdkPath[80] sits immediately after cTimeZone[16] in DCMSettingsHandle; a ≥16-byte TimeZoneMode JSON value overwrites cRdkPath with shell metacharacters. The popen() sink is confirmed at dcm.c:109-113 → dcm_utils.c:96 with no v_secure_system or quoting. High rather than Critical because remote vector requires trusted-infra compromise. |
| 3 | High-003 (DCM-003) | Unbounded strcpy() of cloud-supplied JSON string values into tiny fixed heap buffers (8/16/128 bytes) | High | Exploitable — any JSON string value exceeding the destination buffer size triggers heap corruption in the root daemon; even legitimate cron expressions like "0 0,6,12,18 * * *" (17 bytes) overflow logCron[16] | CWE-122: Heap-based Buffer Overflow | dcm_parseconf.c:224 does strcpy(sval, pJsonItem->valuestring) with no length check. Five callers pass cUploadPrtl[8], cUploadURL[128], cTimeZone[16], logCron[16], difdCron[16]. The 2048-byte fgets cap on the JSON does NOT prevent overflowing these tiny buffers. Downgraded from Critical because network vector requires operator-infra compromise. |
| 4 | High-004 (DCM-004) | Heap overflow of cTimeZone[16] corrupts adjacent cRdkPath[80] → root command injection via popen() | High | Exploitable — end-to-end chain confirmed: cloud/RBUS JSON → strcpy overflow → cRdkPath corruption → popen() as root. Attacker also controls the cron schedule that triggers the popen() | CWE-78: OS Command Injection | Struct layout places cTimeZone[16] immediately before cRdkPath[80] with no padding (both INT8 arrays). A ~100-byte JSON payload suffices: TimeZoneMode overflow + CheckSchedule:cron for immediate trigger. Blocked on devices with ENABLE_MAINTENANCE set in /etc/device.properties. |
| 5 | High-005 (DCM-005) | 1-byte destination stack buffer passed to strcpy() when UploadOnReboot is sent as a JSON string instead of expected boolean | High | Exploitable — any string value ≥1 byte overflows the 1-byte stack slot, corrupting adjacent locals and potentially saved registers; stack canary on hardened builds converts RCE to DoS | CWE-121: Stack-based Buffer Overflow | INT8 temp=0 (1 byte on stack) passed as sval to dcmSettingJsonGetVal(); no type enforcement means a JSON string triggers strcpy into a single byte. Adjacent-local corruption (pJsonHandle, type, uploadCheck) occurs before any canary check. Downgraded from Critical because remote vector requires XCONF compromise and stack-protector-strong likely present. |
| 6 | High-006 (DCM-006) | popen() wrapper executes caller-supplied string via /bin/sh with no validation, escaping, or whitelisting | High | Exploitable — this is the terminal sink for the highest-impact attack chain (cloud JSON → heap overflow → cRdkPath → popen as root). No v_secure_system, no quoting, no allowlist | CWE-78: OS Command Injection | dcmUtilsCopyCommandOutput() at dcm_utils.c:96 passes cmd verbatim to popen(cmd,"r"). The sole production caller builds the command with unescaped pRDKPath. The direct /etc/include.properties vector is weak (root-owned file), but the indirect heap-overflow vector via cTimeZone is fully confirmed. |
| 7 | High-007 (DCM-007) | Static-buffer overflow in attempt_proxy_fallback() while parsing server-supplied S3 URL — .bss overflow in root process | High | Exploitable — path_len can reach ~1013 bytes, overflowing static char clean_path[512] by ~500 attacker-controlled bytes. Network vector requires mTLS server compromise; local vector via /tmp/httpresults.txt tamper is reliable | CWE-787: Out-of-bounds Write | strncpy(clean_path, path_start, path_len) with no bounds check against sizeof(clean_path). Reachable when device_type=="mediaclient" and PROXY_BUCKET is set. The local /tmp vector exploits the world-writable httpresults.txt file (created mode 0666 due to umask(0)). |
| 8 | High-008 (DCM-008) | Unbounded strcpy of network-sourced JSON values into 8/16/128-byte struct fields — heap metadata corruption | High | Exploitable — duplicate root cause of DCM-003; difdCron[16] is the last field of malloc'd DCMDHandle, so overflow runs off the heap chunk into allocator metadata. Even benign cron strings (18 bytes) corrupt memory | CWE-120: Classic Buffer Overflow | Same strcpy sink at dcm_parseconf.c:224. The trailing-field overflow of difdCron makes heap-metadata corruption deterministic. Input reaches this path from RBUS Device.DCM.Setconfig or XCONF cloud JSON. Downgraded from Critical because strongest practical path is local RBUS → root heap overflow. |
| 9 | High-009 (DCM-009) | Stack overflow of t2_ver[32] from RBUS-supplied string via unbounded strcpy in dcmRbusGetT2Version() | High | Exploitable — attacker controls both length and full byte content of the overflow; any RBUS provider returning ≥32 bytes triggers stack corruption in the root daemon's init path | CWE-120: Classic Buffer Overflow | strcpy(t2_ver, stringValue) at dcm_rbus.c:327 into a 32-byte stack buffer. The RBUS parameter is remotely writable via WebPA/RFC and persists across reboot. Requires local foothold or operator-side cloud access. Distro-level stack-protector converts RCE to DoS on hardened builds. |
| 10 | High-010 (DCM-010) | Directory symlink following in add_directory_to_tar() allows arbitrary root-readable file exfiltration via uploaded archive | High | Exploitable — local unprivileged user plants symlink in /tmp/DCM/ (world-writable); root daemon follows it via stat(), archives target contents, and uploads to cloud endpoint | CWE-59: Improper Link Resolution Before File Access | stat() at archive_manager.c:619 follows symlinks on directory entries; O_NOFOLLOW at :525 only protects regular files. /tmp/DCM/ is deleted after each upload, giving attacker a non-racy window to recreate and own it. The resulting tarball is also written back to source_dir, enabling local readback. |
| 11 | High-011 (DCM-011) | gzopen() on predictable path under /tmp without O_NOFOLLOW/O_EXCL enables arbitrary file overwrite as root | High | Exploitable — fully predictable filename (MAC + 1-minute timestamp) in world-writable /tmp subdir; symlink at target path causes root to truncate/overwrite arbitrary file with gzip data | CWE-377: Insecure Temporary File | gzopen(archive_path,"wb9") follows symlinks with O_CREAT\|O_TRUNC. fs.protected_symlinks does not help because the symlink sits in an attacker-owned non-sticky subdir of /tmp, not in sticky /tmp itself. Content control is limited (gzip stream), so impact is primarily file clobber / DoS / integrity loss. |
| 12 | Medium-001 (DCM-012) | Stack buffer overflow in dcmRbusGetT2Version() via unbounded strcpy() of RBUS-supplied string | Medium | Exploitable with constraints — same root cause as DCM-009 but fires only once at daemon init; requires winning RBUS provider registration race against legitimate RFC agent | CWE-121: Stack-based Buffer Overflow | Downgraded from High because: (a) init-time-only path narrows trigger window, (b) attacker must be registered RBUS provider at boot time, (c) RDK Yocto's -fstack-protector-strong places canary in dcmDaemonMainInit (has char[32] local), converting RCE to DoS on hardened builds. |
| 13 | Medium-002 (DCM-013) | popen() executed on uninitialised heap buffer when DCM_LOG_UPLOAD scheduler fires before DCM_FW_UPDATE | Medium | Partially Exploitable — the core CWE-457 defect is confirmed but the claimed RCE via heap spray is speculative; no attacker-content-controlled allocation exists in the init path before pExecBuff is allocated | CWE-457: Use of Uninitialized Variable | malloc(1024) not zeroed at dcm.c:192; LOG_UPLOAD branch never writes pExecBuff but popen() at :113 is unconditional. Fresh sbrk pages are zero-filled (popen("") is a no-op). Realistic impact is DoS/unintended swupdate re-execution, not RCE. Fix: one-line calloc() or memset(). |
| 14 | Medium-003 (DCM-014) | umask(0) makes every file created by root daemon world-writable (mode 0666/0777) | Medium | Exploitable — any local user can tamper /tmp/DCMSettings.conf to suppress log uploads (anti-forensics) or rewrite PID file to "1" for persistent DoS of dcmd on restart | CWE-732: Incorrect Permission Assignment for Critical Resource | umask(0) at dcm.c:295 has no functional justification (copy-paste daemonization pattern). Affects ~12 files including /tmp/.dcm-daemon.pid, /tmp/DCMSettings.conf, /opt/rdk_maintenance.conf. Impact is integrity tampering / DoS, not code execution. The /opt URL-redirect claim is not exercised by code in this repo. |
| 15 | Medium-004 (DCM-015) | Stack buffer overflow in dcmSettingSaveMaintenance() when cron minute/hour token ≥ 4 chars — even legal cron fields overflow | Medium | Exploitable with constraints — even a legitimate cron field like '*/15' (4 chars) fills buffmin[4] with no NUL terminator; fields ≥5 chars write past the stack arrays. Requires HAS_MAINTENANCE_MANAGER build + ENABLE_MAINTENANCE runtime | CWE-121: Stack-based Buffer Overflow | buffhr[4]/buffmin[4] written via *ptr++ with no bounds check. Input capped to ~15 bytes by intermediate difdCron[16] buffer (still 11 bytes past buffmin[4]). Stack canaries on typical RDK builds convert to deterministic DoS. Downgraded from High because input requires operator-controlled XCONF/T2 or local RBUS. |
| 16 | Medium-005 (DCM-016) | Negative-index heap write before cJsonStr when 'urn:settings:TelemetryProfile' appears at offset 0/1 — heap metadata corruption | Medium | Not Exploitable for RCE — writes two FIXED bytes (0x7D, 0x00) at FIXED offsets into glibc chunk header; modern glibc aborts on size-mismatch check. DoS only | CWE-787: Out-of-bounds Write | pcJsonStr[lc-2]='}'/ pcJsonStr[lc-1]=0 with no bounds check on lc. cJsonStr is the first member of malloc'd DCMSettingsHandle, so negative indices land in heap chunk header. The corrupted chunk is allocated once at init and never freed in the while(1) loop, so corruption stays dormant until shutdown. |
| 17 | Medium-006 (DCM-017) | Arbitrary file read as root via attacker-controlled confPath — no path validation (realpath, prefix check, or symlink check) | Medium | Exploitable with constraints — enables arbitrary-file-open-as-root and is the enabling primitive for all overflow findings; info leak is constrained to files parseable as JSON. Delivering the event requires RBUS provider registration race | CWE-73: External Control of File Name or Path | fopen(confPath,"r") at dcm_parseconf.c:134,257 with zero path sanitization. Also enables DoS via FIFO/huge file and pivots to all heap/stack overflows. Downgraded from High because info-leak is JSON-only, overflow chain tracked separately, and RBUS delivery is not trivial. |
| 18 | Medium-007 (DCM-018) | Root writes /tmp/DCMSettings.conf via fopen("w") without O_NOFOLLOW/O_EXCL — symlink clobber + world-writable 0666 file | Medium | Partially Exploitable — symlink-clobber vector is blocked by fs.protected_symlinks=1 (default since Linux 3.6, standard on RDK/Yocto); 0666-tamper vector allows upload suppression / anti-forensics only | CWE-59: Improper Link Resolution Before File Access | fopen("/tmp/DCMSettings.conf","w") follows symlinks; umask(0) makes new files 0666. Written content is server-derived (not attacker-chosen), so symlink follow yields destructive truncate only. The 0666-tamper lets any uid flip upload boolean — integrity tampering, not code execution. |
| 19 | Medium-008 (DCM-019) | Unchecked CRON_INVALID_INSTANT (-1) return causes infinite tight loop firing root popen() — CPU exhaustion DoS | Medium | Exploitable for DoS — a valid-parse but impossible-date cron expression like "0 0 31 2 *" (fits in 16-byte buffer) causes the scheduler thread to fire dcmRunJobs() in a tight loop with no delay | CWE-835: Infinite Loop | dcmCronParseGetNext() returns (time_t)-1 for impossible dates; scheduler never checks this, sets tv_sec=-1, and pthread_cond_timedwait() returns ETIMEDOUT immediately. Each iteration is synchronous (popen/pclose blocks), so this is continuous serialized root-script execution, not a fork-bomb. Impact limited to availability (CPU, log growth, flash wear). |
| 20 | Medium-009 (DCM-020) | Symlink-follow on /tmp/.dcm-daemon.pid fopen("w") — arbitrary file truncate/create as root with mode 0666 | Medium | Partially Exploitable — blocked by fs.protected_symlinks=1 (default on modern kernels); requires boot-time race to place symlink before dcmd starts. If exploitable, enables arbitrary root-file truncation and creation of world-writable root-owned files | CWE-59: Improper Link Resolution Before File Access | fopen("/tmp/.dcm-daemon.pid","w") with no O_NOFOLLOW/O_EXCL/lstat. umask(0) means a followed symlink to non-existent path creates a root-owned 0666 file. Sibling components (cleanup_handler.c, backup_engine.c) deliberately use O_NOFOLLOW, confirming this is an oversight. Downgraded from High due to kernel symlink protection and local-only timing requirement. |
